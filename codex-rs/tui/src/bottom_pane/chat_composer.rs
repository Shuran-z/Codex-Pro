...
pub(crate) struct ChatComposer {
    ...
    status_line_model_click_width: Option<u16>,
    last_status_line_model_hitbox: std::cell::Cell<Option<Rect>>,
...
impl ChatComposer {
    ...
    pub(crate) fn new_with_config(
        ...
        let mut this = Self {
            ...
            status_line_model_click_width: None,
            last_status_line_model_hitbox: std::cell::Cell::new(None),
            history_search: None,
    ...
    pub(crate) fn set_status_line_model_click_width(&mut self, width: Option<u16>) {
        self.status_line_model_click_width = width;
        if width.is_none() {
            self.last_status_line_model_hitbox.set(None);
        }
    }

    pub(crate) fn status_line_model_hitbox_contains(&self, x: u16, y: u16) -> bool {
        self.last_status_line_model_hitbox
            .get()
            .is_some_and(|hitbox| {
                x >= hitbox.x
                    && x < hitbox.x.saturating_add(hitbox.width)
                    && y >= hitbox.y
                    && y < hitbox.y.saturating_add(hitbox.height)
            })
    }
...
impl ChatComposer {
    ...
    fn update_status_line_model_hitbox(&self, area: Rect, line: &Line<'_>) {
        let Some(width) = self.status_line_model_click_width else {
            self.last_status_line_model_hitbox.set(None);
            return;
        };
        let left_padding = FOOTER_INDENT_COLS as u16;
        let max_width = area.width.saturating_sub(left_padding);
        let visible_line_width = (line.width() as u16).min(max_width);
        let width = width.min(visible_line_width);
        if area.height == 0 || width == 0 {
            self.last_status_line_model_hitbox.set(None);
            return;
        }

        self.last_status_line_model_hitbox.set(Some(Rect::new(
                            ...
                            SummaryLeft::Default => {
                                if status_line_active {
                                    if let Some(line) = truncated_status_line.clone() {
                                        self.update_status_line_model_hitbox(hint_rect, &line);
                                        render_footer_line(hint_rect, buf, line);
                                    } else {
