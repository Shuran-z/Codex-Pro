...
impl ChatWidget {
    ...
    fn refresh_status_line_from_selections(&mut self, selections: &StatusSurfaceSelections) {
        ...
        if !enabled {
            ...
            self.bottom_pane.set_status_line_model_click_width(None);
            return;
        }
        ...
        let model_click_width = segments.first().and_then(|(item, value)| {
            matches!(
                item,
                StatusLineItem::ModelName | StatusLineItem::ModelWithReasoning
            )
            .then(|| u16::try_from(UnicodeWidthStr::width(value.as_str())).unwrap_or(u16::MAX))
        });
        self.bottom_pane
            .set_status_line_model_click_width(model_click_width);
